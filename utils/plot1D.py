...
def eval_acc(model, bx, by, bt, args):
    model.eval()
    accs = np.zeros(args.n_tasks, dtype=np.float32)
...
def eval_loss(model, bx, by, bt, args, criterion):
    model.eval()
    losses = np.zeros(args.n_tasks, dtype=np.float32)
...
def eval_unif(old_param, param_list, model):
    unif_loss_temp = 0
    for i in range(len(param_list)):
        input = torch.cat([old_param[i].view(1,-1), param_list[i].view(1,-1)])
        output = model.LN(input)
        unif_loss_temp += uniform_loss(output, t=2)
    unif_loss = unif_loss_temp.cpu() / len(param_list)
    return unif_loss
...
def calculate_loss(model, data, ids, task, steps, args):
    ...
    with torch.no_grad():
        ...
        for t in range(task + 1):
            ...
            acc = eval_acc(model, xvalid, yvalid, tvalid, args)
        ...
        train_acc = eval_acc(model, train_x, train_y, train_t, args)


        for di in range(args.dir_num):
            direction = create_random_direction(model.parameters(), args=args, model=model)
            for s, step in enumerate(steps):
                model = set_weights(model, trained_weights, direction, step)

                train_lss[:, di, s] = eval_loss(model, train_x, train_y, train_t, args, criterion)
