...
class AlexNet(nn.Module):
    def __init__(self,taskcla):
        ...
        self.add_fc = nn.Linear(2048,64, bias=False)
        self.LN = L2Norm()
...
class RobustNet(nn.Module):
    def __init__(self,device):
        ...
        self.Layer1 = BaseNet_robust([64,3,4,4],device)
        self.Layer2 = BaseNet_robust([128,64,3,3], device)
        self.Layer3 = BaseNet_robust([256,128,2,2], device)
        self.Layer4 = BaseNet_robust([2048,1024],device)
        self.Layer5 = BaseNet_robust([2048,2048],device)

def robust_train(Net, robust_optimizer, param_list):
    new_param_list=[]

    unif_loss, new_param = Net.Layer1(param_list[0])
    new_param_list.append(new_param)
    unif_loss.backward()
    robust_optimizer.step()
    robust_optimizer.zero_grad()

    unif_loss, new_param = Net.Layer2(param_list[1])
    new_param_list.append(new_param)
    unif_loss.backward()
    robust_optimizer.step()
    robust_optimizer.zero_grad()

    unif_loss, new_param = Net.Layer3(param_list[2])
    new_param_list.append(new_param)
    unif_loss.backward()
    robust_optimizer.step()
    robust_optimizer.zero_grad()

    unif_loss, new_param = Net.Layer4(param_list[3])
    new_param_list.append(new_param)
    unif_loss.backward()
    robust_optimizer.step()
    robust_optimizer.zero_grad()

    unif_loss, new_param = Net.Layer5(param_list[4])
    new_param_list.append(new_param)
    unif_loss.backward()
    robust_optimizer.step()
    robust_optimizer.zero_grad()
...
def train(args, model, device, x, y, optimizer, criterion, task_id):
    ...
    for i in range(0,len(r),args.batch_size_train):
        ...
        output2 = model(mix_inputs)[task_id]#, f=True
...
def main(args):
    ...
    if args.visual_landscape:
        ...
        visual_lss = np.zeros((10, 10, args.dir_num, len(steps)), dtype=np.float32) #n_task=10
        visual_val_acc = np.zeros((10, 10), dtype=np.float32)
        visual_train_acc = np.zeros((10, 10), dtype=np.float32)
    ...
    for k,ncla in taskcla:
        ...
        if task_id==0:
            ...
            Net = RobustNet(device)
            robust_optimizer = optim.SGD([{'params':Net.Layer1.parameters()},
        ...
        if args.visual_landscape and task_id<=9:
            visual_lss[k], visual_val_acc[k], visual_train_acc[k] = plot.calculate_loss(model, data, ids, k, steps, args)
        # if task_id == 4 and args.visual_landscape:
        #     numbers = np.arange(-0.5, 0.5, 0.02)  
        #     numbers_list = numbers.tolist() 
        #     df1 = pd.DataFrame(visual_lss[4][4], columns=numbers_list)   
        #     df1.to_csv('output_rcl.csv', index=False, header=True)
    ...
    if args.visual_landscape:
        ...
        file_name = '%s_ep_%d_task_%d_%s' % ('dfgp', args.n_epochs, args.n_tasks, timestamp)
        plot.plot_1d_loss_all(visual_lss, steps, file_name, show=True)
        plot.save_visual_results(visual_val_acc, visual_train_acc, acc_matrix, file_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--savename', type=str, default='./logs/CIFAR100/',
                        help='save path')
    parser.add_argument('--gpm_thro', type=float, default=0.94, metavar='gradient projection',
                        help='gpm_thro')
    ...
    parser.add_argument('--step_min', type=float, default=-0.5)
    parser.add_argument('--step_max', type=float, default=0.5)
    parser.add_argument('--step_size', type=float, default=0.02)
    parser.add_argument('--dir_num', type=int, default=3)
    parser.add_argument('--n_tasks', type=int, default=10,
                        help='The task of drawing weight loss landscape')
    parser.add_argument('--cuda', default=True, action='store_true', help='Use GPU')
    
    #robust parameters
    parser.add_argument('--ua_weight', type=float, default=1, metavar='ua_Weight',
                        help='ua_weight')
    ...
    parser.add_argument('--robust_epoch', type=float, default=200, metavar='N',
    ...
    for ua_weight in [1]:
        for weight_range in [0.0001]:

            accs, bwts = [], []
            args.ua_weight = ua_weight
            args.weight_range = weight_range

            str_time = str_time_ + '_' + str(ua_weight) +  '_' + str(weight_range)
            ...
            for seed_ in [1,2]:
                ...
                log.info('time cost ={}'.format(str(time.time() - train_begin_time)))
                accs.append(acc)
                bwts.append(bwt)
